...
const FileViewerModal = ({ file, onClose }) => {
  const renderFileContent = () => {
    if (file.type.includes('image/')) {
      ...
      return (
        <section className="flex flex-col items-center justify-center p-8 text-gray-500">
  ...
  return (
    <dialog open className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <article className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] flex flex-col">
        ...
        <section className="p-6 overflow-auto flex-grow" aria-label="File content preview">
          {renderFileContent()}
        </section>
